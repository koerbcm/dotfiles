...
# not sure if the change to gls will work on all systems - 5-5-2025
preexec() { LS_USED=$(echo $1|cut -d' ' -f1) && GLS_INSTALLED=$(which gls) }
chpwd() {
  # if gls is installed, use it
  if [[ -n $GLS_INSTALLED ]]; then
    case "$LS_USED" in
      ...
      *) gls --color=auto --group-directories-first -hF;;
    esac
  else
    case "$LS_USED" in
      cd)         ls -hF;;
...
dkb() {
  ...
  if [ ! -f package.json ]; then
    ...
    read "?Are you in the right directory? "
  fi
